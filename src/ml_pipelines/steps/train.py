...
def init_mlflow_experiment_and_run_config(cfg: DictConfig):
    ...
    print(f"[train:init] experiment={exp_used} tracking_uri={mlflow.get_tracking_uri()} parent_run_id={parent_run_id}")
    return exp_used, parent_run_id


def load_previous_artifacts_for_train(cfg: DictConfig, data: Dict[str, pd.DataFrame] | None):
    if data is not None:
        return data["train"]
    ...
    return load_parquet_artifact_as_df(prep_run_id, "prepare_data/train.parquet")


def train(cfg: DictConfig, exp_used: str, parent_run_id: str | None, train_df: pd.DataFrame):
...
def main(cfg: DictConfig):
    ...
    train_df = load_previous_artifacts_for_train(cfg, None)
    train(cfg, exp_used, parent_run_id, train_df)
